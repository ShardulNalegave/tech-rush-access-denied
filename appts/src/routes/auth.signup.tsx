...
import { login, signUp } from '../query/auth';
...
function AuthSignUpPage() {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    let res = await queryClient.fetchQuery(signUp({ name, email, password }));
    if (!res) {
      alert("Couldn't create account");
      return;
    }

		res = await queryClient.fetchQuery(login({ email, password }));
		if (res === true) {
			window.location.replace('/feed');
		} else {
      alert("Account created, but login failed. Please try again!");
    }
  };
			...
			<div className="relative rounded-lg h-[70vh] max-w-[80vw] w-full bg-white p-10 shadow-lg">
              ...
              <div className='h-[15px]'></div>
                ...
                <div className="flex items-center h-12 w-full relative">
										<i className="fas fa-solid fa-user absolute left-4 text-yellow-500" />
										<input
											type="text"
											name="name"
											value={name}
